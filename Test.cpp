...
TEST_CASE("Constructor Error: Row/col must be greater than 0."){ // 6 tests
...
TEST_CASE("Arithmetic Operators: (+, -, *)"){ // 6 tests
    const std::vector<double> vec = {1,1,1,1,1,1}; //6 elements
    ...
    CHECK_THROWS(test1 - test2); //dimensions don't equal (-)
    CHECK_THROWS(test3 * test2); //dimensions don't equal (*)
    ...
    CHECK_NOTHROW(test2 + test3); //good + case
    CHECK_NOTHROW(test2 - test3); //good - case
    CHECK_NOTHROW(test1 * test3); //good * case
    CHECK_NOTHROW(5*test3); //good num*mat case
    CHECK_NOTHROW(test3*5); //good mat*num case
...
TEST_CASE("Assignment Operators: (+=, -=, *=)"){ //6 tests
    ...
    CHECK_THROWS((test2 += test1)); //dimensions don't equal (+=)
    CHECK_THROWS((test2 -= test1)); //dimensions don't equal (-=)
    CHECK_THROWS((test2 *= test2)); //dimensions don't equal (*=)
    
    CHECK_NOTHROW((test2 += test2)); //check compile (+=)
    CHECK_NOTHROW((test3 -= test3)); //check compile (-=)
    CHECK_NOTHROW((test3 *= test1)); //check compile (*=)
...
TEST_CASE("Relational Operators (<, >, <=, >=, ==, !=):"){
    ...
    CHECK_THROWS(bool x = (test2 == test4)); //dimensions don't equal (==)
    CHECK_THROWS(bool x = (test2 != test4)); //dimensions don't equal (+=)
    CHECK_THROWS(bool x = (test2 >= test4)); //dimensions don't equal (>=)
    CHECK_THROWS(bool x = (test2 <= test4)); //dimensions don't equal (<=)
    CHECK_THROWS(bool x = (test2 > test4)); //dimensions don't equal (<)
